...
- Its stable nav page id, matching a nav page node.
- Its page title.
...
Represent navigation and page generation as explicit data in `site.typ` rather than trying to infer it from page files. All nav nodes need a stable `id` so included pages, breadcrumbs, and navigation components can look up nav information by id.
...
- `id`: a globally unique stable identifier used by navigation components.
...
- `pages-from-nav(nav)`: recursively collect page nodes in site order for document emission.
- `entry-by-id(id, nav)`: recursively look up a page or section node and return `(entry: node, trail: ids)`, where `trail` is an array of nav node ids from the root to the matched node.
- `emit-documents(nav)`: internal helper used by `jtd.site(...)` to emit documents from `pages-from-nav(nav)`.

Validation of nav shape should be handled by Elembic types where practical, not by a separate public validation helper. Semantic checks such as globally unique ids and page path uniqueness can happen while casting/processing the nav tree.
...
Breadcrumbs should be custom elements/components that perform their own nav lookups. They can use the id trail returned by `entry-by-id(...)` and resolve each id back to a node when rendering labels and links. A separate breadcrumb helper is unnecessary for v1.
